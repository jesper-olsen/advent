...
const PANIC_AT_CLOSING_TIME: &str = "A mysterious recorded voice groans into life and announces: \"This exit is closed. Please leave via main office.\"";
...
fn debug(g: &mut Game, m: &str) {
    ...
    g.wrt(&format!("###{m} motions: {v:?}\n"))
}

fn shortest_route(g: &mut Game, x: Loc, y: Loc) {
    ...
    g.wrt(&format!("Shortst Route {x:?} -> {y:?}\n"));
...
impl Game {
    ...
    fn wrt(&mut self, s: &str) {
        //print!("{s}");
        self.tx.send(String::from(s)).unwrap();
    }
    ...
    fn listen(&mut self) -> Vec<String> {
        loop {
            self.wrt(&format!("[{:?}] * ", self.loc));
            //let _ = io::stdout().flush();
            ...
            match words.len() {
                0 => self.wrt(" Tell me to do something.\n"),
                1 | 2 => return words,
                _ => self.wrt(" Please stick to 1 and 2 word commands.\n"),
            }
        }
    }

    fn panic_at_closing_time_180(&mut self) {
        if !self.panic {
            self.clock2 = 15;
            self.panic = true;
        }
    ...
    fn offer(&mut self, j: usize) {
        self.hinted[j] = if j > 1 {
            if !self.yes(HINTS[j].question, "I am prepared to give you a hint", OK) {
                return;
            }
            let s = format!(
            ...
            self.yes(&s, HINTS[j].answer, OK)
        } else {
            self.yes(HINTS[j].question, HINTS[j].answer, OK)
        };
    ...
    pub fn game_loop(&mut self) {
        self.offer(0);
        self.limit = if self.hinted[0] { 1000 } else { 330 };
        ...
        loop {
            state = match state {
                Goto::Branch => branch(self),
                Goto::GetObject => get_object(self),
                Goto::TryMove => try_move(self),
                Goto::CantSeeIt => cant_see_it(self),
                Goto::Major => major(self),
                Goto::Minor => minor(self),
                Goto::Cycle => cycle(self),
                Goto::Death => death(self),
                Goto::Commence => commence(self),
                Goto::Transitive => transitive(self),
                Goto::Intransitive => intransitive(self),
                Goto::Parse => parse(self),
                Goto::PreParse => pre_parse(self),
                Goto::GoForIt => go_for_it(self),
            }
        }
    } // game_loop
    ...
    fn score(&self) -> i32 {
        ...
        for o in TREASURES {
            if self.prop[o] >= 0 {
                ...
                if self.is_at(o, Loc::House) && self.prop[o] == 0 {
        ...
        s += 10 * (MAX_DEATHS - self.death_count) as i32;
        if !self.gave_up {
            s += 4
        };
        if self.is_at(Obj::Mag, Loc::Witt) {
...
fn death(g: &mut Game) -> Goto {
    ...
    g.death_count += 1;
    if g.closing() {
        g.prn("It looks as though you're dead. Well, seeing as how it's so close to closing time anyway, let's just call it a day.");
        g.quit()
    }
    if !g.yes(
...
fn pitch_dark(g: &mut Game) -> Goto {
    g.prn("You fell into a pit and broke every bone in your body!");
    g.oldoldloc = g.loc;
    death(g)
}

fn dwarves_upset(g: &mut Game) -> ! {
    g.prn("The resulting ruckus has awakened the dwarves. There are now several threatening little dwarves in the room with you! Most of them throw knives at you! All of them get you!");
    g.quit()
}
...
fn major(g: &mut Game) -> Goto {
        ...
        && (1..g.dloc.len()).any(|j| g.dloc[j] == g.newloc && g.dseen[j])
    {
        ...
        g.newloc = g.loc;
        g.prn("A little dwarf with a big knife blocks your way. \u{1F479}");
    }
        ...
        match g.dflag {
            ...
            1 => {
                if g.loc >= MIN_LOWER_LOC && pct(5) {
                    ...
                    g.prn("A little dwarf just walked around a corner, saw you, threw a little axe at you, cursed, and ran away. (The axe missed.)");
...
fn cycle(g: &mut Game) -> Goto {
    ...
    if g.closed {
        if g.prop[Obj::Oyster] < 0 && g.toting(Obj::Oyster) {
            g.prn(Obj::Oyster.note(g.prop[Obj::Oyster]));
        }
...
fn pre_parse(g: &mut Game) -> Goto {
    ...
    if g.clock1 == 0 {
        ...
        g.prn("A sepulchral voice, reverberating through the cave, says, \"Cave closing soon. All adventurers exit immediately through main office.\"");
        g.clock1 = -1;
        g.prop[Obj::Grate] = 0;
        ...
        if g.clock2 == 0 {
            //⟨ Close the cave 181 ⟩ ≡
            g.prn("The sepulchral voice intones, \"The cave is now closed.\" As the echoes fade, there is a blinding flash of light (and a small puff of orange smoke). . . . Then your eyes refocus; you look around and find...");
            for (obj, p) in [
                (Obj::Bottle, -2),
                (Obj::Plant, -1),
                (Obj::Oyster, -1),
                ...
                && g.here(Obj::Lamp)
            {
                ...
                g.prop[Obj::Lamp] = 0;
                if g.here(Obj::Lamp) {
                    g.prn("Your lamp has run out of power.");
                }
            } else if g.limit < 0 && g.loc < MIN_IN_CAVE {
                g.prn("There's not much point in wandering around out here, and you can't explore the cave without a lamp. So let's just call it a day.");
                g.gave_up = true;
                g.quit()
                ...
                g.prn(&format!("Your lamp is getting dim{}", s));
                g.warned = true;
            }
        }
    }
...
fn parse(g: &mut Game) -> Goto {
    ...
    if g.words[0] == "west" {
        g.west_count += 1;
        if g.west_count == 10 {
            g.prn(" If you prefer, simply type W rather than WEST.");
        }
    }
            ...
            if !g.toting(*o) && !g.is_here(*o) {
                match o {
                    ...
                    Obj::Knife if g.loc == g.knife_loc => {
                        g.knife_loc = Loc::Limbo;
                        g.prn("The dwarves' knives vanish as they strike the walls of the cave.");
                    }
...
fn transitive(g: &mut Game) -> Goto {
    let s=match g.verb {
        ...
        Act::Break if g.obj == Obj::Mirror => {
            if g.closed {
                g.prn("You strike the mirror a resounding blow, whereupon it shatters into a myriad tiny fragments.");
                dwarves_upset(g)
            } else {
                "It is too far up for you to reach"
            }
        ...
        Act::Wake if g.closed && g.obj == Obj::Dwarf => {
            g.prn("You prod the nearest dwarf, who wakes up grumpily, takes one look at you, curses, and grabs for his axe.");
            dwarves_upset(g)
        }
            ...
            if !g.toting(g.obj) {
                ...
                if g.is_here(Obj::Plant) {
                    ...
                    if g.obj != Obj::Water {
                        ...
                        g.prn(Obj::Plant.note(g.prop[Obj::Plant]));
                        g.prop[Obj::Plant] += 2;
...
fn intransitive(g: &mut Game) -> Goto {
    let s = match g.verb {
        ...
        Act::Score => {
            ...
            g.gave_up = g.yes("Do you indeed wish to quit now?", OK, OK);
            if g.gave_up {
                g.quit();
            }
            ""
        ...
        Act::Quit => {
            g.gave_up = g.yes("Do you really wish to quit now?", OK, OK);
            if g.gave_up {
                g.quit();
            }
            ""
...
fn commence(g: &mut Game) -> Goto {
    ...
    if g.toting(Obj::Bear) {
        g.prn("You are being followed by a very large, tame bear.");
    }
    g.wrt("\n");
    g.prn(s);
    ...
    if g.loc == Loc::Y2 && pct(25) && !g.closing() {
        g.prn("A hollow voice says \"PLUGH\".")
    }
    if !g.dark() {
        ...
        for o in g.l2o[g.loc].clone() {
            ...
            if o != Obj::Treads || g.toting(Obj::Gold) {
                g.prn(if o == Obj::Treads && g.loc == Loc::Emist {
                    o.note(g.prop[o] + 1)
                } else {
                    o.note(g.prop[o])
                })
...
fn go_for_it(g: &mut Game) -> Goto {
    ...
    let s = match g.newloc {
        ...
        Loc::Troll => {
            ...
            if g.toting(Obj::Bear) {
                g.prn("Just as you reach the other side, the bridge buckles beneath the weight of the bear, who was still following you around. You scrabble desperately for support, but as the bridge collapses you stumble back and fall into the chasm.");
                g.prop[Obj::Bridge] = 1;
                g.prop[Obj::Troll] = 2;
